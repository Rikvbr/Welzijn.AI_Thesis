...
def main(Sentiment):
    ...
    for i in range(0,len(LiwcSentList)):
        ...
        if LiwcSentList[i] == "Pos" and BertSentList[i] == "Pos":    
            PosPos = PosPos + 1
        if LiwcSentList[i] == "Pos" and BertSentList[i] == "Und":    
            PosUnd = PosUnd + 1
        if LiwcSentList[i] == "Pos" and BertSentList[i] == "Neg":    
            PosNeg = PosNeg + 1
        if LiwcSentList[i] == "Und" and BertSentList[i] == "Pos":    
            UndPos = UndPos + 1
        if LiwcSentList[i] == "Und" and BertSentList[i] == "Und":    
            UndUnd = UndUnd + 1
        if LiwcSentList[i] == "Und" and BertSentList[i] == "Neg":    
            UndNeg = UndNeg + 1
        if LiwcSentList[i] == "Neg" and BertSentList[i] == "Pos":    
            NegPos = NegPos + 1
        if LiwcSentList[i] == "Neg" and BertSentList[i] == "Und":    
            NegUnd = NegUnd + 1
        if LiwcSentList[i] == "Neg" and BertSentList[i] == "Neg":    
            NegNeg = NegNeg + 1

    # Score voor alleen pos en neg, PosNeg = hij denkt pos maar het is neg
    # Dus pred = Pos en Actual = Neg

    page.append(["Kappa =", PosPos, PosUnd, PosNeg, UndPos, UndUnd, UndNeg, NegPos,  NegUnd, NegNeg])

    def ResultsNegPosUndSingle (SentList):
        PosPos = PosUnd = PosNeg = UndPos = UndUnd =UndNeg = NegPos =  NegUnd = NegNeg = 0
        for i in range(0,len(SentList)):

            if SentList[i] == "Pos" and Sentiment == "Positief":    
                PosPos = PosPos + 1
            if SentList[i] == "Pos" and Sentiment == "Neutraal":    
                PosUnd = PosUnd + 1
            if SentList[i] == "Pos" and Sentiment == "Negatief":    
                PosNeg = PosNeg + 1
            if SentList[i] == "Und" and Sentiment == "Positief":    
                UndPos = UndPos + 1
            if SentList[i] == "Und" and Sentiment == "Neutraal":    
                UndUnd = UndUnd + 1
            if SentList[i] == "Und" and Sentiment == "Negatief":    
                UndNeg = UndNeg + 1
            if SentList[i] == "Neg" and Sentiment == "Positief":    
                NegPos = NegPos + 1
            if SentList[i] == "Neg" and Sentiment == "Neutraal":    
                NegUnd = NegUnd + 1
            if SentList[i] == "Neg" and Sentiment == "Negatief":    
                NegNeg = NegNeg + 1

        # page.append(["Los model", PosPos, PosUnd, PosNeg, UndPos, UndUnd, UndNeg, NegPos,  NegUnd, NegNeg])

    def ResultsNegPosUndBoth (LiwcSentList,BertSentList):
        SentList = []
        PosPos = PosUnd = PosNeg = UndPos = UndUnd =UndNeg = NegPos =  NegUnd = NegNeg = 0
        for i in range(0,len(LiwcSentList)):
            ...
            if LiwcSentList[i] == BertSentList[i]:
                ...
                SentList.append(BertSentList[i])
            elif (LiwcSentList[i] != "Und" and BertSentList[i] == "Und") :  
                SentList.append(BertSentList[i])
            elif LiwcSentList[i] == "Und" and BertSentList[i] == "Und":
                SentList.append("Und")
        ...
        # page.append(["beide", PosPos, PosUnd, PosNeg, UndPos, UndUnd, UndNeg, NegPos,  NegUnd, NegNeg])

    ResultsNegPosUndSingle(BertSentList)
    ResultsNegPosUndSingle(LiwcSentList)
    ResultsNegPosUndBoth(LiwcSentList,BertSentList)
